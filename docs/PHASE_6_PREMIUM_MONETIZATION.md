...
| **D** | **30-day money-back guarantee mechanism**. Snapshot baseline application counts at subscription start; cron job at day 30 evaluates lift; auto-refund + cancel if lift < threshold. Email both ways. | Add `baseline_application_count`, `evaluated_at`, `guarantee_triggered` columns on `subscriptions` | ~5 hrs |
