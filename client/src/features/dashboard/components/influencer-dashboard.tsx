...
import { Users, TrendingUp, Wallet, BarChart3, Plus, Eye, Heart, Share2, Play, Instagram, Youtube, MessageSquare, Star, Folder } from "lucide-react";
        ...
        <div className="relative z-10 space-y-8">
            ...
            <div className="bg-gradient-to-br from-purple-100/80 to-indigo-200/60 backdrop-blur-lg rounded-2xl border border-white/20 shadow-lg hover:shadow-xl transition-all duration-300 p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-slate-800">{(totalFollowers / 1000).toFixed(1)}K</p>
                  <p className="text-sm text-purple-600 font-medium">{followerGrowth > 0 ? '+' : ''}{followerGrowth}% growth</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-orange-100/80 to-amber-200/60 backdrop-blur-lg rounded-2xl border border-white/20 shadow-lg hover:shadow-xl transition-all duration-300 p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-slate-800">{engagementRate.toFixed(1)}%</p>
                  <p className="text-sm text-orange-600 font-medium">Average engagement</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-green-100/80 to-emerald-200/60 backdrop-blur-lg rounded-2xl border border-white/20 shadow-lg hover:shadow-xl transition-all duration-300 p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-green-600 font-medium">Creating content</p>
                </div>
          ...
          <div className="bg-white/70 backdrop-blur-lg rounded-2xl border border-white/20 shadow-lg p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-800">Recent Social Activity</h2>
            ...
            {recentSocialActivities.length > 0 ? (
              <div className="space-y-4">
                {recentSocialActivities.slice(0, 5).map((activity: any, index: number) => (
              ...
              <div className="text-center py-12 text-slate-500">
                ...
                <p className="font-medium">No recent social activity</p>
                <p className="text-sm">Start creating content to see your engagement here</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white/70 backdrop-blur-lg rounded-2xl border border-white/20 shadow-lg p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-800">Top Content Clippers</h2>
              <Button variant="outline" size="sm">
                View Directory
              </Button>
            </div>
            
            {topInfluencerClippers.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {topInfluencerClippers.slice(0, 6).map((clipper: any, index: number) => (
                  <div key={index} className="p-4 border border-pink-200 rounded-lg bg-gradient-to-br from-pink-50 to-rose-50 hover:shadow-md transition-shadow">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-600">{clipper.platform}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-slate-600">Views:</span>
                        <span className="ml-1 font-semibold text-pink-600">{clipper.totalViews}</span>
                      </div>
                      <div>
                        <span className="text-slate-600">Engagement:</span>
                        <span className="ml-1 font-semibold text-purple-600">{clipper.engagementRate}%</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-12 text-slate-500">
                ...
                <p className="text-sm">Create a campaign to attract content clippers</p>
              </div>
            )}
          </div>
        </div>
